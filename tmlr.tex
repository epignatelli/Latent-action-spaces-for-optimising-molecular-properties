...
\newcommand{\tom}[1]{\textcolor{brown}{\textbf{tom: #1}}}
...
SMILES-based representation creates a duality between text and molecules. Each molecule $m$ can be represented by at least one SMILES string $s_m$\footnote{Although multiple SMILES strings may represent the same molecule, we use a canonical SMILES representation as in RDKit \texttt{Chem.MolToSmiles}, which is unique for a given molecular graph under a fixed canonicalization procedure and fixed choices regarding stereochemistry, charges, aromaticity, and tautomeric/protonation state.}, allowing chemical structures to be treated as sequences of characters.
...
% Tommy you can take the RL background from here if you want (no need to) https://arxiv.org/pdf/2312.01072
% or maybe here is shorter: https://arxiv.org/pdf/2409.12798
% But again, no need to!! Feel free to ignore it and make it yours!
\tom{sure}
